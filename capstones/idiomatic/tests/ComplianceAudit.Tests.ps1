...
Describe 'Idiomatic capstone scaffold' -Tag Smoke {
    ...
    It 'fails unfinished behavior with the intentional scaffold error' {
        ...
        try {
            Import-CompliancePolicy -Path (
                Join-Path -Path $TestDrive -ChildPath 'policy.json'
            )
        }
        catch {
            $caught = $_
    ...
    It 'parses the selected convenience launcher skeleton' {
        $tokens = $null
        $errors = $null
    ...
    It 'binds launcher arguments before failing intentionally' {
        $caught = $null
        try {
            & $script:target.ScriptPath 'test' '--policy' './policy.json'
        }
        catch {
            $caught = $_
        }
